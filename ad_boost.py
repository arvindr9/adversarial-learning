...
class AdversarialBoost:
	...
	n_estimator : number of estimators in boosting loop
	learning_rate : (float)
	to_add_adv : (int) 1 if to be trained with adversarial examples
	...
	def __init__(self, base_estimator, n_estimators, learning_rate, to_add_adv, epsilon, n_adv, n_boost_random_train_samples, estimator_params = None):
...
if(__name__ == '__main__'):
	...
	ab = AdversarialBoost(SVC(), no_boosting_clf, learning_rate, 1, epsilon_train, n_advimages, n_boost_random_train_samples)
	...
	ab_n = AdversarialBoost(SVC(), no_boosting_clf, learning_rate, 0, epsilon_train, n_advimages, n_boost_random_train_samples)
	...
	adv_examples_test_adv, adv_true_adv = ab._advGen(ab, 100, 200, epsilon_test, eval_data_ss, eval_labels_ss)
	adv_examples_test_norm, adv_true_norm = ab_n._advGen(ab_n, 100, 200, epsilon_test, eval_data_ss, eval_labels_ss)


	print("Overall testing accuracy on adversarial images with adversarial training: {}".format(accuracy_score(adv_true_adv, ab.predict(adv_examples_test_adv))))
	print("Overall testing accuracy on adversarial images without adversarial training: {}".format(accuracy_score(adv_true_norm, ab_n.predict(adv_examples_test_norm))))

	#Results for each weak classifiers
	...
	for est_adv, est_norm in zip(ab.estimators_, ab_n.estimators_):
		...
		adv_examples_test_adv, adv_true_adv = ab._advGen(est_adv, 50, 100, epsilon, eval_data_ss, eval_labels_ss)
		adv_examples_test_norm, adv_true_norm = ab_n._advGen(est_norm, 50, 100, epsilon, eval_data_ss, eval_labels_ss)
	...
	plt.savefig('normal_accuracy_' + clf + '.png')
	...
	plt.savefig('adv_accuracy_' + clf + '.png')
